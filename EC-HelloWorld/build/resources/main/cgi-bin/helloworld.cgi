...
# helloworld.cgi -
#
# Get/set EC-HelloWorld configuration info for UI
#
...
################################
# getCfgList - Return the list of configurations for EC-HelloWorld
#
# Arguments:
#   -
#
...
sub getCfgList {
    ...
    my $gcfg = new ElectricCommander::PropDB($::ec,"/projects/EC-HelloWorld-1.0.0.0/helloworld_cfgs");
    ...
    foreach my $cfg (keys  %cfgs) {
        my $formalLevel = $gcfg->getCol("$cfg/formalLevel");
        $xml .= "  <cfg>\n";
        $xml .= "     <name>$cfg</name>\n";
        $xml .= "     <formalLevel>" . xmlQuote($formalLevel) . "</formalLevel>\n";
        $xml .= "  </cfg>\n";
    }
    $xml .= "</cfgs>\n";
